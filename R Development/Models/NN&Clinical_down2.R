...
datos_exp <- read.csv(file = "ClinicalOutcomesDS/CO_atlas2018_down2.csv", stringsAsFactors = FALSE)
...
data <- merge(x = categorical_data, y = datos_exp[,c("X","PDLIM5", "OSBPL10", "EGFR", "FRMD4B", "POFUT2", "HFE", "KCNMA1", "MOG", "ZNF44", "DOCK10", "VDR", "CA12", "RUNX1", "TNFRSF25", "SKAP2","OsStatus")], by = "X", all.x = TRUE)
categorical_data$STAGE <- as.factor(categorical_data$STAGE)
...
data_matrix <- model.matrix(~AgeRange+SEX+RACE+ETHNICITY+RepeteadTumor+STAGE+
                              ...
                              PDLIM5+OSBPL10+EGFR+FRMD4B+POFUT2+HFE+KCNMA1+MOG+ZNF44+DOCK10+VDR+CA12+RUNX1+TNFRSF25+SKAP2+OsStatus, data=data)
