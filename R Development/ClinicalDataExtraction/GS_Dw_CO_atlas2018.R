...
FIRMA <- "../gene signature/firma_down_1.xlsx"
...
RNA_geneSign<-merge(x=datos_geneSign, y=datos_RNA, by.x="Gene", by.y="Entrez_Gene_Id", all.x=TRUE)
RNA_geneSign<-RNA_geneSign[!is.na(RNA_geneSign$Hugo_Symbol), ]
t1<-data.frame(t(RNA_geneSign))

rownames(t1)<-colnames(RNA_geneSign)
colnames(t1)<-RNA_geneSign$Hugo_Symbol
...
write.csv(x = datos_exp, file = "ClinicalOutcomesDS/CO_atlas2018_down1.csv")
